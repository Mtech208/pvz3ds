...
if /I "!ARG!"=="clean" (
    ...
    REM Accepted and ignored: it used to select a second preset, wii-game, that
    REM this repository never had. The split came from the port this script was
    REM adapted from, where "no argument" built a small bring-up test and "game"
    REM built the real thing. Here there is one Wii preset and it always builds
    REM the full game, so the keyword resolves to wii-release like everything
    REM else. Kept rather than removed so existing habits and the
    REM "build wii - game.bat" shortcut keep working instead of erroring out
    REM with "No such preset in ...: wii-game".
    set PRESET=wii-release
) else if /I "!ARG!"=="data" (
    set DODATA=1
) else (
