...
#ifdef PS2_PLATFORM
// See Reanimator.cpp for the rationale: park the audio mixer around main-thread
// file I/O so its audsrv SIF RPCs don't collide with the reads and wedge the
// IOP on real hardware. One park wraps the whole per-level preload burst below.
namespace Sexy { void Ps2MixerIoPauseBegin(); void Ps2MixerIoPauseEnd(); }
namespace {
	struct Ps2CutSceneIoPause {
		Ps2CutSceneIoPause()  { Sexy::Ps2MixerIoPauseBegin(); }
		~Ps2CutSceneIoPause() { Sexy::Ps2MixerIoPauseEnd(); }
	};
}
#define PS2_PAUSE_MIXER_FOR_IO() Ps2CutSceneIoPause aPs2CutSceneIoPause_
#else
...
void CutScene::PreloadSeedBankResources()
{
	...
	for (int anIndex = 0; anIndex < mBoard->mSeedBank->mNumPackets; anIndex++)
	{
		...
		// Everything past NUM_SEED_TYPES is a pseudo-seed with no gPlantDefs entry
		// (Beghouled/Slot Machine buttons, Zombiquarium purchases, iZombie
		// zombies), so it must never reach PreloadPlantResources: its
		// GetPlantDefinition() would index past the end of the array.
		if (aPacket.mPacketType >= SeedType::NUM_SEED_TYPES)
		{
			// Only the iZombie packets map to a zombie. IsZombieSeedType() also
			// claims the two Zombiquarium ones, but IZombieSeedTypeToZombieType()
			// has no case for either and asserts on them — the snorkel packet buys
			// a ZOMBIE_SNORKEL and the trophy buys no zombie at all.
			if (aPacket.mPacketType == SeedType::SEED_ZOMBIQUARIUM_SNORKLE)
			{
				Zombie::PreloadZombieResources(ZombieType::ZOMBIE_SNORKEL);
			}
			else if (aPacket.mPacketType != SeedType::SEED_ZOMBIQUARIUM_TROPHY &&
				Challenge::IsZombieSeedType(aPacket.mPacketType))
			{
...
void CutScene::PreloadResources()
{
	...
	// One park for the whole per-level preload: it reads every level plant/zombie
	// reanim + images on the main thread while the selector music is live. Nested
	// reanim loads re-enter the park cheaply (it stays raised throughout), so
	// there is no park thrash. Without it each first-use fio can wedge the IOP.
...
void CutScene::CancelIntro()
{
	...
	if (mCutsceneTime > mCrazyDaveTime + TimePanLeftStart || !mBoard->ChooseSeedsOnCurrentLevel())
	{
		...
		if (mApp->IsFinalBossLevel())
		{
			mApp->mMusic->StartGameMusic(mBoard);
		}
...
void CutScene::UpdateZombiesWon()
{
	...
	{
#ifdef LOW_MEMORY
		// Same rationale as Board::PlayerLoses's ZOMBIES_WON load: the board
		// is at its heaviest here and this definition is never preloaded
		// under LOW_MEMORY, so reclaim unreferenced assets before the
		// on-demand load right below.
...
void CutScene::ClearUpsellBoard()
{
	...
	mBoard->InvalidatePlantGridCache();
#endif
