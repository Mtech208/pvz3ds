...
void Sexy::PrintF(const char *text, ...)
{
	...
	// Common.h maps the engine's printf calls to PrintF, so writing to stdout
	// here bypassed the Wii linker's printf wrapper and ignored SEXY_LOG_LEVEL.
	// Use the shared gated logger: level 0 drops the line, while enabled levels
	// reach the file/Gecko sinks without painting into the game framebuffer.
	SexyLogPrintf("%s", str);
#else
...
namespace
{
	struct Ps2MkDirIoPause
	{
		Ps2MkDirIoPause()  { Sexy::Ps2MixerIoPauseBegin(); }
		~Ps2MkDirIoPause() { Sexy::Ps2MixerIoPauseEnd(); }
	};
}
#endif
...
{
#if defined(PS2_PLATFORM)
	// Each mkdir() below is a fio RPC; PlayerInfo::SaveDetails and others
	// call this on the main thread with the mixer live. Nesting under
	// WriteBytesToFile's park is fine (the pause is refcounted).
	Ps2MkDirIoPause aMixerPause;
#endif
