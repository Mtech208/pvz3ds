#ifndef __PS2GRAPHICSINTERFACE_H__
#define __PS2GRAPHICSINTERFACE_H__
...
#include "graphics/Ps2GsTexture.h"
...
typedef std::set<GLImage*> Ps2ImageSet;
...
struct VertexList
{
	...
	Ps2Vertex mStackVerts[MAX_STACK_VERTS];
	Ps2Vertex* mVerts;
	int mSize;
	int mCapacity;
	...
	// mSize arranca en 0 a proposito: reserve() copia mSize verts del buffer
	// viejo, y aqui el buffer viejo es mStackVerts (vacio). Fijar mSize antes
	// de reserve() leia theList.mSize verts sin inicializar del stack.
	...
	void reserve(int theCapacity)
	{
		if (mCapacity < theCapacity)
		{
			...
			Ps2Vertex* aNewList = new Ps2Vertex[theCapacity];
	...
	void push_back(const Ps2Vertex& theVert)
	{
		if (mSize == mCapacity)
			reserve(mCapacity * 2);
...
public:
	...
	int mImageFlags;
	// Draws of this image to skip before retrying a failed CreateTextures
	// (decode refused or a native texture allocation failed). Per-image, so one
	// stuck image can't starve another's retry and a burst of failures can't
	// re-decode every frame.
	int mCreateFailCooldown;
	...
	Ps2GsTexture* GetTexture(int x, int y, int &width, int &height, float &u1, float &v1, float &u2, float &v2);
	Ps2GsTexture* GetTextureF(float x, float y, float &width, float &height, float &u1, float &v1, float &u2, float &v2);
...
#endif // __PS2GRAPHICSINTERFACE_H__
