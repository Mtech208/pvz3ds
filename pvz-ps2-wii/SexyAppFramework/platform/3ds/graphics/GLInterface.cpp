...
static C3D_RenderTarget* gfxTarget;              // top screen = game ("gfx")
static C3D_Tex* gCurrentBoundTex = NULL;
...
static void SetTexture(C3D_Tex *theTex, bool force);
...
static void GfxEnsureCapacity(int count)
{
	if (gStartVertex + gNumVertices + count >= MAX_VERTICES)
	{
		...
		SetTexture(gCurrentBoundTex, true);
...
static void GfxAddVertices(const GLVertex *arr, int arrCount)
{
	...
	GfxEnsureCapacity(arrCount);
	
	for (int i = 0; i < arrCount; i++)
	{
		gVertices[gStartVertex + gNumVertices + i] = arr[i];
	}
...
static void GfxAddVertices(VertexList &arr)
{
	...
	GfxEnsureCapacity((int)arr.size());
...
static void ToMortonTexture32(C3D_Tex* tex, u32* dst, u32* src, int originX, int originY, int width, int height)
{
	...
	for (int y = 0; y < height; y++)
	{
		...
		for (int x = 0; x < width; x++)
		{
			...
			// This is the layout the PvZ-Portable 3DS fork ships and the
			// PICA200 factory shaders consume (byte0 = alpha): do not "fix" it
			// to straight RGBA8, that only made Azahar render worse.
			u8 r = pixel & 0xff;
			u8 g = (pixel >> 8) & 0xff;
			u8 b = (pixel >> 16) & 0xff;
			u8 a = (pixel >> 24) & 0xff;
...
void TextureData::CreateTextureDimensions(MemoryImage *theImage)
{
	...
	// BUGFIX: when mTexVecHeight==1, mTexVecWidth*(mTexVecHeight-1)==0 which
	// starts the loop at index 0 and overwrites ALL piece widths (including the
	// right-edge piece that was just correctly set) with aBottomWidth.  Guard
	// with mTexVecHeight>1 because a single-row image has no separate bottom row.
	if (mTexVecHeight > 1)
	{
	...
	// Assign corner piece (last entry = bottom-right corner)
	// Only meaningful when the image extends past one piece in both dimensions.
	if (mTextures.size() > 1)
	{
...
void TextureData::CreateTextures(MemoryImage *theImage)
{
	...
	for(y=0; y<aHeight; y+=mTexPieceHeight)
	{
		for(x=0; x<aWidth; x+=mTexPieceWidth, i++)
		{
			...
			if (createTextures)
			{
				if (!C3D_TexInit(&aPiece.mTexture, aPiece.mWidth, aPiece.mHeight, aFormatType))
				{
					C3D_TexDelete(&aPiece.mTexture);
					TodTraceAndLog("[TEX] C3D_TexInit FAILED %dx%d fmt=%d linfree=%u -> skipping image", aPiece.mWidth, aPiece.mHeight, (int)aFormatType, (unsigned)linearSpaceFree());
					aPiece.mTexture.data = NULL;
				}
				else
					mTexMemSize += aPiece.mWidth*aPiece.mHeight*aFormatSize;
			}

			if (aPiece.mTexture.data != NULL)
...
void GLInterface::UpdateViewport()
{
	...
	// Game renders on the TOP screen (400x240 physical). The 4:3 viewport below
	// restricts drawing to a 320x240 area centered in it, so the game world
	// (800x600 -> 320x240 presentation rect) maps 1:1 with crisp pixels and
	// 40px black bars on each side. The BOTTOM screen is owned by the console
	// (consoleInit(GFX_BOTTOM) in Window.cpp) and shows the debug log.
	int viewport_x = 0;
	int viewport_y = 0;
	int width = 400;
	int height = 240;
	...
	mPresentationRect = Rect(0, 0, 320, 240);
...
int GLInterface::Init(bool IsWindowed)
{
	...
	if (!inited)
	{
		...
		// Initialize the render target for the TOP screen (240x400 physical hardware orientation).
		// Note: Citro3D render targets must match the hardware framebuffer dimensions:
		// 240 width, 400 height for GFX_TOP. The 4:3 viewport in UpdateViewport centers
		// the 320x240 game area in it.
		gfxTarget = C3D_RenderTargetCreate(240, 400, GPU_RB_RGBA8, GPU_RB_DEPTH24_STENCIL8);
		C3D_RenderTargetSetOutput(gfxTarget, GFX_TOP, GFX_LEFT, DISPLAY_TRANSFER_FLAGS);

		u32 color = ((u8)(0*255) << 24) | ((u8)(0*255) << 16) | ((u8)(0*255) << 8) | 0xFF;
		C3D_RenderTargetClear(gfxTarget, C3D_CLEAR_ALL, color, 0);
...
{
	// Main-thread heartbeat: 240 frames ~= 4 seconds. If this stops printing
	// while the [IP]/[LOADPROBE] lines from the loading thread keep going, the
	// main thread is stuck in C3D; if both stop, the GPU itself wedged.
	static int sFrameCount = 0;
	if ((++sFrameCount % 240) == 1)
		TodTraceAndLog("[HB] frame %d alive", sFrameCount);
	...
	// Fully clear the 240x400 target before the game redraws so the frame
	// never shows stale pixels from an earlier swap (on 3DS the display pings
	// between double/quad frame buffers, so an init-time-only clear is not
	// enough; this has to happen every frame).
	C3D_RenderTargetClear(gfxTarget, C3D_CLEAR_ALL, 0x000000FF, 0);
}
