...
#ifdef WII_PLATFORM
#include "platform/wii/WiiEarlyInit.h" // wiiLog for the Prepare() branch probe
#endif
...
static int GetKerningOffset(const CharData* theCharData, SexyChar theNextChar)
{
	CharIntMap::const_iterator anItr = theCharData->mKerningOffsets.find(theNextChar);
	if (anItr == theCharData->mKerningOffsets.end())
		return 0;
...
bool FontData::Error(const std::string& theError)
{
	...
	TodTraceAndLog("[FONTDATA ERROR] %s", anErrorString.c_str());
...
// Pack the character's bytes low-byte-first rather than memcpy'ing them into
// the char32_t. memcpy puts the string's FIRST byte at the LOWEST ADDRESS,
// which is the low 8 bits only on a little-endian CPU -- so this was reading
// the value the file stores through the host's byte order, the one case
// SexyEndian.h exists for.
//
// It mattered because every caller narrows the result to SexyChar, which is a
// char: FontLayer::GetCharData takes one, and CharDataMap is keyed by one. On
// big-endian "L" arrived as 0x4C000000 and truncated to 0, so the whole
// descriptor filed its glyphs under key 0. At draw time nothing matched, the
// rect came back 0x0, and Graphics::DrawImage returned at its zero-size check
// without ever creating a texture -- which is why on Wii every string was
// invisible while sprites drew correctly, and why no font atlas ever appeared
// in the texture log.
//
// The shifts reproduce the little-endian layout on every CPU, so PC and PS2
// keep exactly the values they have today. Capping at sizeof(ret) also stops a
// malformed descriptor entry from writing past the char32_t, which the memcpy
// did for any string longer than four bytes.
//
// This packs UTF-8 bytes instead of decoding a code point, which is
// pre-existing: the lookup side indexes single bytes out of a SexyString, so
// only ASCII was ever addressable on any platform. Left alone deliberately --
// real multi-byte support is a change to both sides, not to this function.
static char32_t UTF8CharToUTF32Char(const std::string &theString)
{
	char32_t ret = 0;
	const size_t aByteCount = std::min(theString.length(), sizeof(ret));
	for (size_t i = 0; i < aByteCount; i++)
		ret |= (char32_t)(unsigned char)theString[i] << (8 * i);
	return ret;
}
...
bool FontData::HandleCommand(const ListDataElement& theParams)
{
	...
	// Packed PC descriptors put a UTF-8 BOM in front of the first command. The
	// reader strips it, but if a probe path ever misses it the token comes out
	// as "<EF><BB><BF>Define" and dispatch fails. Drop the prefix defensively.
	if (!aCmd.empty() && (unsigned char)aCmd[0] == 0xEF)
	{
		aCmd.erase(0, 1);
		if (!aCmd.empty() && (unsigned char)aCmd[0] == 0xBB)
			aCmd.erase(0, 1);
		if (!aCmd.empty() && (unsigned char)aCmd[0] == 0xBF)
			aCmd.erase(0, 1);
	}
	...
	else if (strcasecmp(aCmd.c_str(), "LayerSetImage") == 0)
	{
		if (theParams.mElementVector.size() == 3)
		{
				...
				(DataToString(theParams.mElementVector[2], &aFileNameString)))
			{
				...
				{
					// The packed glyph sheets live as "_X.png" companions (e.g.
					// "data/_BrianneTod16.png" for the 'BrianneTod16' layer), so
					// the extension probe of "data/BrianneTod16" misses them from
					// the pak. Loose-override copies on the SD use the plain
					// name, but from the pak the underscore variant is the only
					// hit. Probe it before giving up on the descriptor.
					std::string anUnderName;
					size_t aSlash = aFileName.rfind('/');
					if (aSlash != std::string::npos)
						anUnderName = aFileName.substr(0, aSlash + 1) + "_" + aFileName.substr(aSlash + 1);
					else
						anUnderName = "_" + aFileName;
					anImage = mApp->GetSharedImage(anUnderName, "", &isNew);
				}
					...
					if ((Image*)aLayer->mImage != NULL)
					{
						...
						for (uint32_t i = 0; i < aCharsVector.size(); i++)
						{
...

							{
								...
								if (aRect.mX >= anImageWidth) aRect.mX = std::max(0, anImageWidth - 1);
								if (aRect.mY >= anImageHeight) aRect.mY = std::max(0, anImageHeight - 1);

								if (aRect.mX + aRect.mWidth > anImageWidth)
									aRect.mWidth = std::max(0, anImageWidth - aRect.mX);
								if (aRect.mY + aRect.mHeight > anImageHeight)
									aRect.mHeight = std::max(0, anImageHeight - aRect.mY);
...
{
	TodTraceAndLog("[FONTDATA] Load start '%s'", theFontDescFileName.c_str());
	if (mInitialized)
		return false;
	...
	TodTraceAndLog("[FONTDATA] Load '%s' -> mInitialized=%d", theFontDescFileName.c_str(), (int)mInitialized);

	// If descriptor load failed, clear any partially allocated define map
	// before caller tries LoadLegacy, to avoid memory leaks
	if (!mInitialized)
	{
		DataElementMap::iterator anItr = mDefineMap.begin();
		while (anItr != mDefineMap.end())
...
{
	TodTraceAndLog("[FONTDATA] LoadLegacy start '%s'", theFontDescFileName.c_str());
	if (mInitialized)
		return false;
	...
	aFontLayer->mDefaultHeight = ((MemoryImage*)aFontLayer->mImage != NULL) ? ((MemoryImage*)aFontLayer->mImage)->GetHeight() : 0;
	aFontLayer->mAscent = aFontLayer->mDefaultHeight;
	...
	if (aStream == NULL)
	{
		TodTraceAndLog("[FONTDATA] LoadLegacy '%s' p_fopen returned NULL", theFontDescFileName.c_str());
		return false;
	}

	mSourceFile = theFontDescFileName;

	int aSpaceWidth = 0, aAscent = 0;
	char aLine[256] = {0};
	if (p_fgets(aLine, sizeof(aLine), aStream) != NULL)
	{
		sscanf(aLine, "%d%d", &aSpaceWidth, &aAscent);
		aFontLayer->GetCharData(' ')->mWidth = aSpaceWidth;
		aFontLayer->mAscent = aAscent;
	}
	...
	while (!p_feof(aStream))
	{
		...
		if (p_fgets(aLine, sizeof(aLine), aStream) == NULL) break;
		if (sscanf(aLine, "%1s%d", aBuf, &aWidth) < 2) continue;
		aChar = aBuf[0];

		if (aChar == 0)
			break;

		int aHeight = ((MemoryImage*)aFontLayer->mImage != NULL) ? ((MemoryImage*)aFontLayer->mImage)->GetHeight() : 0;
		aFontLayer->GetCharData(aChar)->mImageRect = Rect(aCharPos, 0, aWidth, aHeight);
...
ImageFont::ImageFont(SexyAppBase* theSexyApp, const std::string& theFontDescFileName)
{
	...
	mFontData->Ref();
	if (!mFontData->Load(theSexyApp, theFontDescFileName))
	{
		TodTraceAndLog("[IMAGEFONT] Descriptor load failed for '%s', trying LoadLegacy", theFontDescFileName.c_str());
		mFontData->LoadLegacy(NULL, theFontDescFileName);
	}
...
ImageFont::ImageFont(Image* theFontImage, const std::string& theFontDescFileName)
{
	...
	mFontData = new FontData();
	mFontData->Ref();
	if (!mFontData->Load(gSexyAppBase, theFontDescFileName))
	{
		mFontData->LoadLegacy(theFontImage, theFontDescFileName);
	}
	else if (theFontImage != NULL && !mFontData->mFontLayerList.empty())
	{
		for (auto& aLayer : mFontData->mFontLayerList)
		{
			if ((MemoryImage*)aLayer.mImage == NULL)
				aLayer.mImage = (MemoryImage*)theFontImage;
		}
	}
	mPointSize = mFontData->mDefaultPointSize;
	GenerateActiveFontLayers();
...
#if defined(PS2_PLATFORM) || defined(WII_PLATFORM)
	mActiveLayerList.reserve(mFontData->mFontLayerList.size());
#endif
...
#ifdef WII_PLATFORM
				// Which of the two branches below a font takes decides whether it
				// draws from the shared atlas texture or from a rescaled copy
				// built by software blits. Identify the font by its atlas width
				// (1686 renders today; 2636 and 3516 do not) and report the
				// inputs that pick the branch. Capped so repeated Prepare() calls
				// cannot flood.
				{
					static int sPrepLogged = 0;
					if (sPrepLogged < 16)
					{
						sPrepLogged++;
						Image* aLayerImage = (Image*)aFontLayer->mImage;
						const bool aFastPath = (mScale == 1.0) &&
							((aFontLayer->mPointSize == 0) || (mPointSize == aFontLayer->mPointSize));
						wiiLog("[WII][PREP] atlas=%dx%d scale=%d.%03d ptSize=%d layerPt=%d -> %s\n",
							aLayerImage ? aLayerImage->mWidth : -1,
							aLayerImage ? aLayerImage->mHeight : -1,
							(int)mScale, (int)((mScale - (int)mScale) * 1000),
							mPointSize, aFontLayer->mPointSize,
							aFastPath ? "SHARED" : "RESCALED(software)");
					}
				}
#endif
...
int ImageFont::CharWidthKern(SexyChar theChar, SexyChar thePrevChar)
{
	...
	while (anItr != mActiveLayerList.end())
	{
		...
		if (aLayerPointSize == 0)
		{
			...
			if (aPrevCharData != nullptr)
				aSpacing = (anActiveFontLayer->mBaseFontLayer->mSpacing + GetKerningOffset(aPrevCharData, theChar)) * mScale;
			else
				aSpacing = 0;
#else
		...
		else
		{
			...
			if (aPrevCharData != nullptr)
				aSpacing = (anActiveFontLayer->mBaseFontLayer->mSpacing + GetKerningOffset(aPrevCharData, theChar)) * aPointSize / aLayerPointSize;
			else
				aSpacing = 0;
#else
	...
	for (uint32_t aCharNum = 0; aCharNum < theString.length(); aCharNum++)
	{
		...
		while (anItr != mActiveLayerList.end())
		{
			...
			if (aScale == 1.0)
			{
				...
				if (aNextChar != 0)
					aSpacing = anActiveFontLayer->mBaseFontLayer->mSpacing + GetKerningOffset(aCharData, aNextChar);
				else
					aSpacing = 0;
#else
			...
			else
			{
				...
				if (aNextChar != 0)
					aSpacing = (int)((anActiveFontLayer->mBaseFontLayer->mSpacing + GetKerningOffset(aCharData, aNextChar)) * aScale);
				else
					aSpacing = 0;
#else
...
#if defined(PS2_PLATFORM) || defined(WII_PLATFORM)
			int anOrder = layerOrderOffset + anActiveFontLayer->mBaseFontLayer->mBaseOrder + aCharData->mOrder;
#else
			...
			// Note for whoever ends up here chasing missing text: the mSrc lines
			// below read the rect with operator[], which default-inserts a 0x0
			// Rect for a character the layer does not have. That makes a missing
			// glyph draw nothing at all rather than fail loudly -- Graphics::
			// DrawImage returns at its zero-size check without even creating the
			// atlas texture. If text disappears again, check membership with
			// find() BEFORE these lines; afterwards every lookup reports a hit.
