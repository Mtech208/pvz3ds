...
#ifdef PS2_PLATFORM
#include "Lawn/Ps2BoardControls.h"
#endif
#ifdef PS2_PLATFORM
...
#endif
#ifdef WII_PLATFORM
#include "SexyAppFramework/misc/GraphicsBlockArena.h"
#endif
...
// and sounds right after (visible as a burst of hitches). Keep the
// platform-specific pressure policy here so gameplay code never needs to
// know about MEM2, the Wii graphics arena, mallinfo(), or PS2 heap limits.
bool LawnApp::ShouldPurgeAssets()
{
#ifdef PS2_PLATFORM
	return Ps2ShouldPurgeAssets();
#else
	return true;
#endif
}

// Runtime levels need a stricter policy than screen transitions. In
// particular FINAL_BOSS is long-lived and streams many zombie reanimations
// without destroying the Board in between.
bool LawnApp::IsRuntimeMemoryUnderPressure()
{
	if (IsHeapUnderPressure())
		return true;

#ifdef PS2_PLATFORM
	if (Ps2ShouldPurgeAssets())
		return true;
#endif

#ifdef WII_PLATFORM
	// The texture shim uses a dedicated RAM arena that is invisible to the
	// normal heap-pressure heuristic. Reclaim before fragmentation makes a
	// medium atlas impossible to place.
...
static void LawnPurgeUnusedReanimationAssets(LawnApp* theApp)
{
	if (theApp->mReanimatorCache != nullptr)
		theApp->mReanimatorCache->ReanimatorCachePurge();
	ReanimatorFreeUnusedDefinitions();
}

// Lightweight in-level collector. It deliberately keeps Image objects and
// live textures that are still referenced. ReanimatorFreeUnusedDefinitions
// releases orphan source textures, while lazy CPU-side image bits are cheap
// and transparently reloadable.
void LawnApp::PurgeRuntimeAssets()
{
#ifdef WII_PLATFORM
	const size_t aGraphicsBefore = ConsoleGraphicsUsedBytes();
	const size_t aLargestBefore = ConsoleGraphicsLargestFreeBytes();
#endif

	LawnPurgeUnusedReanimationAssets(this);

#ifdef WII_PLATFORM
	// Dead reanimation definitions release their atlases, but a long boss
	// level can still retain reloadable source textures long after their last
	// draw. Evict only cold textures and cap the work per sweep.
	int aColdPurged = 0;
	if (ConsoleGraphicsLargestFreeBytes() < (2u << 20))
		aColdPurged = PurgeColdLazyTextures(500, 12, 4u << 20);
	ConsoleGraphicsCompactFreeBlocks();

	const size_t aGraphicsAfter = ConsoleGraphicsUsedBytes();
	const size_t aLargestAfter = ConsoleGraphicsLargestFreeBytes();
	printf("[WII][GC] cold=%d gfx=%uKB->%uKB largest=%uKB->%uKB\n",
		aColdPurged,
		(unsigned)(aGraphicsBefore / 1024), (unsigned)(aGraphicsAfter / 1024),
		(unsigned)(aLargestBefore / 1024), (unsigned)(aLargestAfter / 1024));
#elif defined(PS2_PLATFORM)
	// Same LRU policy on PS2, with a smaller batch to avoid upload bursts.
	PurgeColdLazyTextures(500, 8, 0);
#endif

	// Pressure was already established; CPU-side bits are independently
	// reloadable and can be reclaimed in the same maintenance pass.
	PurgeLazyImageBits(true);
}

// In-level maintenance policy for low-memory consoles. Normal levels only
// inspect pressure occasionally, so stable stages pay effectively no GC cost.
// FINAL_BOSS streams many zombie/reanimation assets and therefore checks more
// frequently. The collector itself still runs only when pressure is detected.
void LawnApp::UpdateRuntimeMemoryMaintenance(int theMainCounter)
{
	const int aCheckInterval = IsFinalBossLevel() ? 200 : 600;
	if (theMainCounter <= 0 || (theMainCounter % aCheckInterval) != 0)
		return;

	if (!IsRuntimeMemoryUnderPressure())
		return;

	PurgeRuntimeAssets();
}

// Full transition collector. This can be more aggressive because the heavy
// screen/board that owned the discarded assets has already been torn down.
// theForce bypasses the transition pressure heuristic.
void LawnApp::PurgeUnusedAssets(bool theForce)
{
	if (!theForce && !ShouldPurgeAssets())
	{
...
void LawnApp::MakeNewBoard()
{
	...
	// Level boundary: release unused menu/cache definitions plus decoded bits
	// and textures BEFORE this level's large contiguous definition/atlas
	// allocations. Freeing adjacent residents now lets malloc coalesce its
	// holes; waiting for the OOM rescue after loading starts is often too late.
	// Everything the new board needs reloads behind the level intro.
	PurgeUnusedAssets(true);
#endif
...
#ifdef PS2_PLATFORM
	// The menu is (re)entered from the heaviest scenes in the game — the
	// credits movie, a level, the award screens. Free their leftovers (reanim
	// definitions, decoded bits, texture copies) BEFORE the selector's own
	// refault storm, so its texture creations aren't the allocations that
	// fail; a creation that failed here used to leave that sprite black.
	// Everything the selector needs re-faults lazily right below.
	PurgeUnusedAssets(true);
#endif
...
#ifdef LOW_MEMORY
	// The pause dialog's newspaper reanimation asks for fresh texture memory
	// while the board (still fully resident behind it) is at its heaviest —
	// worst case on Night levels with tombstones. CreateTextures fails
	// silently on OOM (leaves the sprite undrawn, retried every frame) rather
	// than freeing anything itself, so reclaim genuinely unreferenced assets
	// first.
	PurgeUnusedAssets();
#endif
...
#ifdef WII_PLATFORM
	// FinishCreateUserDialog refuses an empty name, so seed a unique default:
	// the first free "Player N", which also avoids the name-conflict path when
	// more than one profile is made.
	//
	// Text entry does exist now (the shared VirtualKeyboard, raised by
	// SexyAppBase::StartTextInput), so this is no longer what makes the dialog
	// dismissable -- it is a safety net while that keyboard is unproven, and a
	// convenience for players who would rather click OK than type. Drop it once
	// the keyboard has been confirmed on hardware; the cost of keeping it is
	// that naming a profile starts with clearing the default.
	//
	// PS2 deliberately has no equivalent: it used to auto-create a "Player"
	// profile at startup precisely because it had no text entry, and that was
	// removed when the keyboard reached it, so PS2 players name themselves.
	{
		SexyString aDefaultName;
		for (int i = 1; i <= 99; i++)
		{
			aDefaultName = StrFormat(__S("Player %d"), i);
			if (mProfileMgr->GetProfileMap().find(aDefaultName) == mProfileMgr->GetProfileMap().end())
				break;
		}
		aDialog->SetName(aDefaultName);
	}
#endif
	CenterDialog(aDialog, aDialog->mWidth, aDialog->mHeight);
...
#ifdef PS2_PLATFORM
		// Console-style pad controls (cell snap, seed cycling, shovel) feed
		// the board synthesized mouse events; run them just before the board
		// updates so a press acts on this frame's state.
		Ps2UpdateBoardControls(this);
#endif
...
#define WII_LOAD_STAGE(name) wiiLog("[WII][LOAD] %s\n", name)
#endif
...
// bad_alloc forensics: the standard heap (operator new/malloc) lives in the
// process's APPMEM, while textures live in the linear heap (linearAlloc via
// citro3d). An OOM during load means the standard heap was exhausted, so log
// both figures at every load stage to see which one is about to die.
static void Log3dsMemoryState(const char* theStage)
{
	struct mallinfo mi = mallinfo();
	// fordlibks = free bytes inside malloc arena (holes + headroom).
	// linearSpaceFree() = remaining GPU-usable linear heap.
	TodTraceAndLog("[MEM][%s] std heap free=%u KB used=%u KB lin free=%u KB",
		theStage,
		(unsigned int)(mi.fordblks / 1024),
		(unsigned int)(mi.uordblks / 1024),
		(unsigned int)(linearSpaceFree() / 1024));
}
#define WII_LOAD_STAGE(name) Log3dsMemoryState(name)
#endif

#ifndef WII_LOAD_STAGE
#define WII_LOAD_STAGE(name) ((void)0)
#endif
...
void LawnApp::LoadingThreadProc()
{
	...
	WII_LOAD_STAGE("LoaderBar ready");
	...
	WII_LOAD_STAGE("LoadingImages begin");
	LoadGroup("LoadingImages", 9);
	WII_LOAD_STAGE("LoadingImages done");
	LAWN_LOAD_STAGE(LOAD_FONTS);
	WII_LOAD_STAGE("LoadingFonts begin");
	LoadGroup("LoadingFonts", 54);
	WII_LOAD_STAGE("LoadingFonts done");
	...
	WII_LOAD_STAGE("particles begin (pre purge)");
	LAWN_LOAD_STAGE(LOAD_PARTICLES);
	// Particles load right after fonts+images; on 3DS a single forced purge
	// here buys enough standard-heap headroom (decoded bits are large) to keep
	// every compiled particle definition from OOMing the loader, exactly what
	// the post-particles purge at [LOAD] purge before preload no longer catches.
#if defined(__3DS__) || defined(WII_PLATFORM) || defined(PS2_PLATFORM)
	TodTraceAndLog("[LOAD] purge decoded bits ahead of particles");
	PurgeLazyImageBits(true);  // Force purge: IsHeapUnderPressure() returns false on 3DS
	TodTraceAndLog("[LOAD] purge decoded bits ahead of particles ready");
#endif
	TodParticleLoadDefinitions(gLawnParticleArray, LENGTH(gLawnParticleArray));
	TodTraceAndLog("[LOAD] particles ready");
	WII_LOAD_STAGE("particles done");
	...
	WII_LOAD_STAGE("PreloadForUser begin");
	...
	WII_LOAD_STAGE("PreloadForUser done");
...
{
	// AddTodParticle returns nullptr when the particle-systems pool is full;
	// several callers feed its result straight in here, and DataArrayGetID
	// derefs it. ID 0 is the null sentinel ParticleTryToGet already handles.
...
ReanimationID LawnApp::ReanimationGetID(Reanimation* theReanimation)
{
	if (theReanimation == nullptr)
		return ReanimationID::REANIMATIONID_NULL;
