...
{
#if defined(PS2_PLATFORM) || defined(WII_PLATFORM) || defined(__3DS__)
    // A throwing new here is a silent console freeze (PS2/Wii) or a hard
    // crash with std::bad_alloc (3DS), so allocate nothrow first and give
    // large transient definition buffers one cleanup retry.
    // On 3DS the std heap can be as low as 872 KB by the time particles
    // load: the decompression scratch buffer for MelonImpact.xml.compiled
    // is several hundred KB of contiguous allocation. Purging decoded
    // image bits frees that space before a hard OOM is returned.
    void* aPtr = operator new[](theSize, std::nothrow);
    if (aPtr == NULL && gSexyAppBase != NULL)
    {
#ifdef __3DS__
        struct mallinfo mi = mallinfo();
        TodTraceAndLog("[DEF] OOM allocating %d bytes (std free=%u KB, lin free=%u KB) - full purge + retry",
            theSize, (unsigned int)(mi.fordblks / 1024),
            (unsigned int)(linearSpaceFree() / 1024));
#else
        TodTraceAndLog("[DEF] OOM allocating %d bytes - full purge + retry", theSize);
#endif
        gSexyAppBase->PurgeLazyImages();
        aPtr = operator new[](theSize, std::nothrow);
        TodTraceAndLog("[DEF] OOM retry (%d bytes): %s",
            theSize, aPtr != NULL ? "OK" : "STILL FAILED");
    }
    if (aPtr == NULL)
        TodTraceAndLog("[DEF] OOM allocating %d bytes - FAILED", theSize);
#ifdef WII_PLATFORM
    // Preserve Wii's normal fatal path (including requested-size/caller
    // diagnostics) if the cleanup could not produce a contiguous block.
    if (aPtr == NULL)
        aPtr = operator new[](theSize);
#endif
#else
...
void* DefinitionUncompressCompiledBuffer(void* theCompressedBuffer, size_t theCompressedBufferSize, size_t& theUncompressedSize, const SexyString& theCompiledFilePath, void* theDestination, size_t theDestinationCapacity)
{
    ...
    if (theDestination != nullptr && theDestinationCapacity < aHeader->mUncompressedSize)
        return nullptr;
    Bytef* aUncompressedBuffer = theDestination != nullptr
        ? (Bytef*)theDestination
        : (Bytef*)DefinitionAlloc(aHeader->mUncompressedSize);
...
bool DefinitionReadCompiledFile(const SexyString& theCompiledFilePath, DefMap* theDefMap, void* theDefinition)
{
    PerfTimer aTimer;
    aTimer.Start();
    // Some console IO backends require serialized file access.
...
#if defined(PS2_PLATFORM) || defined(WII_PLATFORM) || defined(__3DS__)
    // Console fast path: the pak layer pre-reads the whole decrypted record
    // into pFile->mBuffer (record path), so the compressed bytes are already
    // resident. Decompress straight from it instead of allocating a second
    // multi-MB copy plus a scratch buffer in the fragmented std heap — that
    // double copy is what tipped the 3DS heap at the MelonImpact particle load.
    if (pFile->mRecord != NULL && pFile->mBuffer != NULL && pFile->mPos == 0)
    {
        size_t aCompressedSize = (size_t)pFile->mRecord->mSize;
        const bool aResult = DefinitionReadCompiledStream(
            pFile->mBuffer, aCompressedSize, theDefMap, theDefinition);
        p_fclose(pFile);
        PlatformIoLockRelease();
        if (!aResult)
            TodTrace(__S("Compiled file streaming read failed: %s\n"),
                theCompiledFilePath.c_str());
        return aResult;
    }
#endif
...
#if defined(PS2_PLATFORM) || defined(WII_PLATFORM) || defined(__3DS__)
    // The compressed input is transient too. Keeping it on the general heap
    // left a 1.19 MB contiguous allocation exposed to fragmentation before
    // the uncompressed scratch buffer was even created.
    ConsoleScratchBuffer aCompressedScratch(aCompressedSize);
    const bool aScratchOwnsCompressedBuffer = (bool)aCompressedScratch;
    void* aCompressedBuffer = aScratchOwnsCompressedBuffer
        ? aCompressedScratch.Data() : DefinitionAlloc(aCompressedSize);
#else
...
#if defined(PS2_PLATFORM) || defined(WII_PLATFORM) || defined(__3DS__)
        if (!aScratchOwnsCompressedBuffer)
...
#if defined(PS2_PLATFORM) || defined(WII_PLATFORM) || defined(__3DS__)
    const bool aResult = DefinitionReadCompiledStream(aCompressedBuffer,
        aCompressedSize, theDefMap, theDefinition);
    if (!aScratchOwnsCompressedBuffer)
        delete[] (char *)aCompressedBuffer;
    if (!aResult)
    {
...
#if defined(PS2_PLATFORM) || defined(WII_PLATFORM) || defined(__3DS__)
    CompressedDefinitionHeader* aCompressedHeader = (CompressedDefinitionHeader*)aCompressedBuffer;
    size_t aScratchSize = aCompressedSize >= sizeof(CompressedDefinitionHeader) &&
        aCompressedHeader->mCookie == 0xDEADFED4L
        ? aCompressedHeader->mUncompressedSize : 0;
    ConsoleScratchBuffer aDefinitionScratch(aScratchSize);
    const bool aScratchOwnsUncompressedBuffer = (bool)aDefinitionScratch;
    void* aUncompressedBuffer = DefinitionUncompressCompiledBuffer(
        ...
        aDefinitionScratch.Data(), aDefinitionScratch.Size());
#else
    ...
    if (aUncompressedSize < theDefMap->mDefSize + sizeof(uint)) {
        ...
        if (!aScratchOwnsUncompressedBuffer)
...
bool DefinitionIsCompiled(const SexyString& theXMLFilePath)
{
    ...
    {
#if defined(PS2_PLATFORM) || defined(WII_PLATFORM)
        // On console the source XML normally ships inside the pak, so stat() on
        // the loose filesystem fails and there is no mtime to compare against.
        // The pak is immutable between runs, so a compiled file already present
        // on disk is valid — reuse it instead of recompiling every boot (each
        // reanim compile is tens of seconds, because DefinitionLoadImage
        // decodes every image the reanim references). Only fall through to
        // "can't compile" if the source isn't in the pak either.
        //
        // This is safe even when the loose file is stale or was built on
        // another machine: the schema CRC in DefinitionReadCompiledFile rejects
        // it, the caller falls back to compiling, and the bad file is
        // overwritten. What it does NOT catch is an asset changing underneath
        // an otherwise-valid compiled file — delete compiled/ when you edit
        // reanims or images.
...
bool DefinitionWriteCompiledFile(const SexyString& theCompiledFilePath, DefMap* theDefMap, void* theDefinition) {
    ...
    // Serialize file creation on backends whose IO service is not thread-safe.
    PlatformIoLockAcquire();
    MkDir(aFilePath);
...
float FloatTrackEvaluate(FloatParameterTrack& theTrack, float theTimeValue, float theInterp)
{
    ...
    if (theTrack.mCountNodes == 1 && theTrack.mNodes[0].mLowValue == theTrack.mNodes[0].mHighValue)
        return theTrack.mNodes[0].mLowValue;
#endif
