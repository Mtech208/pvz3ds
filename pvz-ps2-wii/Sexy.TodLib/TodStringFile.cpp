...
//0x518E40
bool TodStringListReadName(const char*& thePtr, std::string& theName, const char* theBase, int theSize)
{
	...
	if (aNameStart == nullptr)  // 如果文本中不存在“[”
	{
		...
		{
			TodTraceAndLog("Failed to find string name");
			return false;
		}
	...
	else
	{
		...
		{
			int aOffset = (int)(aNameStart - theBase);
			TodTraceAndLog("Failed to find ']' near offset %d of %d, first bytes:", aOffset, theSize);
			for (int i = 0; i < 16; i++)
			{
				if (aOffset + i >= theSize) break;
				TodTraceAndLog("  %02X", (unsigned char)theBase[aOffset + i]);
			}
			return false;
		}
		...
		if (theName.size() == 0)
		{
			TodTraceAndLog("Name Too Short at offset %d", (int)(aNameStart - theBase));
			return false;
		}
...
//0x519080
bool TodStringListReadItems(const char* theFileText, int theFileSize)
{
	...
	for (;;)
	{
		if (!TodStringListReadName(aPtr, aName, theFileText, theFileSize))  // 读取一个标签
			return false;
...
bool TodStringListReadFile(const char* theFileName)
{
	...
	if (pFile == nullptr)
	{
		TodTraceAndLog("Failed to open '%s'", theFileName);
		return false;
	}
	...
	bool aSuccess = true;
	int aBytesRead = p_fread(aFileText, sizeof(char), aSize, pFile);
	
	// Detect UTF-16LE BEFORE checking read success - the file might be in pak
	// as UTF-16LE with BOM, and we need to detect it even if read is short
	bool aIsUtf16LE = (aSize >= 2 &&
	    (unsigned char)aFileText[0] == 0xFF && (unsigned char)aFileText[1] == 0xFE);

	if (aBytesRead != aSize)  // 必须是完整读取，否则残缺尾部含“]”缺失的标签会让解析器误报
	{
		TodTraceAndLog("Failed to read '%s' (%d of %d bytes)", theFileName, aBytesRead, aSize);
		aSuccess = false;
	}
	aFileText[aSize] = '\0';

	char* aParseText = aFileText;
	int aParseSize = aSize;

	// 西班牙语版本的 LawnsStrings.txt 在 pak 内是 UTF-16LE（BOM FF FE），但解析器
	// 是逐字节的（strchr 会停在第一个 0x00 高字节）。这里把 UTF-16LE 转成单字节
	// Latin-1：每个 <=U+00FF 的码元保留原值（西语重音字母原样通过），少量标点
	// 映射到 Windows-1252 槽位。之后 StringToWString 按字节值转宽字符，得到与
	// 桌面版一致的码点。
	// Detection moved BEFORE aSuccess check so it works even on short reads.
	if (aIsUtf16LE)
	{
		aParseSize = (aSize - 2) / 2;
		aParseText = new char[aParseSize + 1];
		for (int i = 0; i < aParseSize; i++)
		{
			const unsigned char aLo = (unsigned char)aFileText[2 + i * 2];
			const unsigned char aHi = (unsigned char)aFileText[3 + i * 2];
			const unsigned short aUnit = (unsigned short)aLo | ((unsigned short)aHi << 8);
			if (aUnit <= 0xFF)
				aParseText[i] = (char)aLo;
			else if (aUnit == 0x20AC)
				aParseText[i] = (char)0x80;  // €
			else if (aUnit == 0x2018)
				aParseText[i] = (char)0x91;  // ‘
			else if (aUnit == 0x2019)
				aParseText[i] = (char)0x92;  // ’
			else
				aParseText[i] = (char)'?';
		}
		aParseText[aParseSize] = '\0';
		TodTraceAndLog("UTF-16LE string file '%s' (%d code units)", theFileName, aParseSize);
	}

	if (aSuccess)
	{
		aSuccess = TodStringListReadItems(aParseText, aParseSize);
	}
	p_fclose(pFile);  // 关闭文件流
	delete[] aFileText;
	if (aParseText != aFileText)
		delete[] aParseText;

	TodTraceAndLog("[STRINGS] '%s' verdict size=%d read=%d utf16=%d parseResult=%d",
		theFileName, aSize, aBytesRead, (aParseText != aFileText) ? 1 : 0, aSuccess ? 1 : 0);
